    ...
    public class Order : EntityBase<int>, IAggregateRoot
    {
        ...
        protected override void Validate()
        {
            if (Created == DateTime.MinValue)
                base.AddBrokenRules(OrderBusinessRules.CreatedDateRequired);
            if (Customer == null)
                base.AddBrokenRules(OrderBusinessRules.CustomerRequired);
            if (DeliveryAddress == null)
                base.AddBrokenRules(OrderBusinessRules.DeliveryAddressRequired);
            if (Items == null || Items.Count() == 0)
                base.AddBrokenRules(OrderBusinessRules.ItemsRequired);
            if (Items == null || Items.Count() == 0)
                base.AddBrokenRules(OrderBusinessRules.ItemsRequired);
            if (ShippingService == null)
                base.AddBrokenRules(OrderBusinessRules.ShippingServiceRequired);
        }
